...
def scrape_website(keywords1, keywords2):
    ...
    homepage_url = "https://etenders.gov.in/eprocure/app"
    
    # Visit the homepage to establish a session
    homepage_response = session.get(homepage_url)
    
    # Extract session parameters or cookies
    session_parameters = homepage_response.cookies.get_dict()  # Extract session cookies
    
    # Continue scraping only if the homepage request was successful
            ...
            for row in table.find_all("tr"):
                ...
                if len(columns) >= 3:
                    ...
                    if any(keyword.lower() in organization_name.lower() for keyword in keywords1):
                        ...
                        data = scrape_organization_page(session, organization_name, page_link, keywords2, session_parameters)
...
def scrape_organization_page(session, organization_name, link, keywords, session_parameters):
    filtered_data = []

    # Send a request to the organization page using the same session object
    response = session.get(link, cookies=session_parameters)
...
def home():
    ...
    session = requests.Session()
    return render_template("index.html", filtered_data=filtered_data, session=session)
